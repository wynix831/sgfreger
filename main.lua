...
local hitboxSizeY = 5
local hitboxSizeZ = 8
local hitboxIgnoreSleeper = false
local hitboxIgnoreBot = false
...
CombatGroup:AddToggle("RedictionToggle", { Text = "Manipulation", Default = false, Callback = function(v) redictionEnabled = v end })
...
-- ==================== HITBOX EXPANDER ====================
HitboxGroup:AddToggle("HitboxExpanderToggle", { Text = "Enabled", Default = false, Callback = function(v)
...
HitboxGroup:AddSlider("HitboxSizeX", { Text = "Size X", Default = 8, Min = 1, Max = 10, Rounding = 1, Callback = function(v)
    hitboxSizeX = v
    if hitboxExpanderEnabled then applyHitboxExpander() end
end })

HitboxGroup:AddSlider("HitboxSizeY", { Text = "Size Y", Default = 5, Min = 1, Max = 10, Rounding = 1, Callback = function(v)
    hitboxSizeY = v
    if hitboxExpanderEnabled then applyHitboxExpander() end
end })

HitboxGroup:AddSlider("HitboxSizeZ", { Text = "Size Z", Default = 8, Min = 1, Max = 10, Rounding = 1, Callback = function(v)
    hitboxSizeZ = v
    if hitboxExpanderEnabled then applyHitboxExpander() end
end })

HitboxGroup:AddToggle("HitboxIgnoreSleeper", { Text = "Ignore Sleeper", Default = false, Callback = function(v)
    hitboxIgnoreSleeper = v
    if hitboxExpanderEnabled then applyHitboxExpander() end
end })

HitboxGroup:AddToggle("HitboxIgnoreBot", { Text = "Ignore Bot", Default = false, Callback = function(v)
    hitboxIgnoreBot = v
    if hitboxExpanderEnabled then applyHitboxExpander() end
end })
...
do
    ...
    CarTab:AddToggle("CarNameOutline", { Text = "Name Outline", Default = false, Callback = function(v) vehicleSettings.Car.NameOutline = v end }):AddColorPicker("CarNameOutlineColor", { Default = Color3.new(0,0,0), Callback = function(v) vehicleSettings.Car.NameOutlineColor = v end })
end
...
local function saveOriginalSize(head)
    if head and head:IsA("BasePart") and not originalHitboxSizes[head] then
...
local function applyHitboxExpander()
    for _, model in pairs(workspace:GetChildren()) do
        if model.Name == "Model" and model:FindFirstChild("HumanoidRootPart") then
            local head = model:FindFirstChild("Head")
            if head and head:IsA("BasePart") then
                local isSleeper = isSleeping(model)
                local isBot = not isPlayer(model)
                
                if (hitboxIgnoreSleeper and isSleeper) or (hitboxIgnoreBot and isBot) then
                    if originalHitboxSizes[head] then
                        head.Size = originalHitboxSizes[head]
                    end
                    continue
                end
                
                saveOriginalSize(head)
...
local function restoreHitboxSizes()
    for head, originalSize in pairs(originalHitboxSizes) do
...
workspace.ChildAdded:Connect(function(obj)
    task.spawn(function()
        if obj:IsA("Model") and obj ~= LocalPlayer.Character then
            ...
            -- HitBox Expander для новых игроков
            if hitboxExpanderEnabled then
                task.wait(0.25)
                local head = obj:FindFirstChild("Head")
                if head and head:IsA("BasePart") then
                    local isSleeper = isSleeping(obj)
                    local isBot = not isPlayer(obj)
                    
                    if not ((hitboxIgnoreSleeper and isSleeper) or (hitboxIgnoreBot and isBot)) then
                        saveOriginalSize(head)
                        head.Size = Vector3.new(hitboxSizeX, hitboxSizeY, hitboxSizeZ)
                    end
                end
